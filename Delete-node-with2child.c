#include <stdio.h>
#include <stdlib.h>

struct Btnode {
    int data;
    struct Btnode* left;
    struct Btnode* right;
};

struct Btnode* newNode(int data) {
    struct Btnode* node = (struct Btnode*)malloc(sizeof(struct Btnode));
    node->data = data;
    node->left = node->right = NULL;
    return node;
}

struct Btnode* minValueNode(struct Btnode* node) {
    struct Btnode* current = node;
    while (current && current->left != NULL) {
        current = current->left;
    }
    return current;
}

struct Btnode* deleteNodeWithTwoChildren(struct Btnode* root, int key) {
    if (root == NULL) {
        return root;
    }

    if (key < root->data) {
        root->left = deleteNodeWithTwoChildren(root->left, key);
    } else if (key > root->data) {
        root->right = deleteNodeWithTwoChildren(root->right, key);
    } else {
        if (root->left != NULL && root->right != NULL) {
            struct Btnode* temp = minValueNode(root->right);
            root->data = temp->data;
            root->right = deleteNodeWithTwoChildren(root->right, temp->data);
        }
    }
    return root;
}

void inorder(struct Btnode* root) {
    if (root != NULL) {
        inorder(root->left);
        printf("%d ", root->data);
        inorder(root->right);
    }
}

int main() {
    struct Btnode* root = newNode(50);
    root->left = newNode(30);
    root->right = newNode(70);
    root->left->left = newNode(20);
    root->left->right = newNode(40);
    root->right->left = newNode(60);
    root->right->right = newNode(80);

    printf("Inorder traversal before deletion: \n");
    inorder(root);
    printf("\n");

    root = deleteNodeWithTwoChildren(root, 50);
    printf("Inorder traversal after deletion of node with two children 50: \n");
    inorder(root);
    printf("\n");

    return 0;
}
